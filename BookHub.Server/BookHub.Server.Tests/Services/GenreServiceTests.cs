namespace BookHub.Server.Tests.Services
{
    ...
    public class GenreServiceTests
    {
        ...
        {
            var genres = await this.genreService.NamesAsync();
        ...
        [Fact]
        public async Task DetailsAsyncAsync_ShouldReturnNull_IfIdIsInvalid()
        {
            var invalidId = 142_325_214;
            var genre = await this.genreService.DetailsAsync(invalidId);

            genre.Should().BeNull();
        }

        [Fact]
        public async Task DetailsAsyncAsync_ShouldReturnServiceModel_IfIdIsValid()
        {
            var id = 1;
            var genre = await this.genreService.DetailsAsync(id);
