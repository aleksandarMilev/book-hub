namespace BookHub.Tests.Services
{
    ...
    public class ReadingListServiceTests
    {
        ...
        [Fact]
        public async Task Add_ShouldReturnErrorMessage_IfUserHasMoreThanFiveBooksInThisList()
        {
            this.mockProfileService
                .Setup(x => x.HasMoreThanFiveCurrentlyReading("current-user-id"))
                .ReturnsAsync(true);

            var result = await this.readingListService.Add(1, "CurrentlyReading");
            result.Succeeded.Should().BeFalse();
            result.ErrorMessage.Should().Be("User can not add more than 5 books in the currently reading list!");
        }

        [Fact]
        public async Task Add_ShouldReturnErrorMessage_IfMapEntityAlreadyExist()
        {
            ...
            var result = await this.readingListService.Add(1, "Read");
            result.Succeeded.Should().BeFalse();
            result.ErrorMessage.Should().Be("This book is already added in the user list!");
        }

        [Fact]
        public async Task Add_ShouldReturnTrue_IfMapEntityCreated()
        {
            foreach (var rl in this.data.ReadingLists)
            {
                this.data.Remove(rl);
        ...
        [Fact]
        public async Task Delete_ShouldReturnErrorMessage_IfMapEntityNotFound()
        {
            foreach (var rl in this.data.ReadingLists)
            {
                this.data.Remove(rl);
            ...
            var result = await this.readingListService.Delete(2, "Read");
            result.Succeeded.Should().BeFalse();
            result.ErrorMessage.Should().Be("The book has not been found in the user list!");
        }

        [Fact]
        public async Task Delete_ShouldReturnTrue_IfMapEntityDeleted()
        {
            foreach (var rl in this.data.ReadingLists)
            {
                this.data.Remove(rl);
            ...
            var result = await this.readingListService.Delete(1, "Read");
            result.Succeeded.Should().BeTrue();
            result.ErrorMessage.Should().BeNull();
        }

        [Fact]
        public async Task All_ShouldReturnPaginatedBooks_IfBooksExist()
        {
            foreach (var rl in this.data.ReadingLists)
            {
                this.data.Remove(rl);
            ...
            var result = await this.readingListService.All(userId, status, 1, 2);
        ...
        [Fact]
        public async Task All_ShouldReturnEmptyList_IfNoBooksMatchCriteria()
        {
            foreach (var rl in this.data.ReadingLists)
            {
                this.data.Remove(rl);
        ...
        [Fact]
        public async Task All_ShouldThrowReadingListTypeException_IfStatusIsInvalid()
        {
            foreach (var rl in this.data.ReadingLists)
            {
                this.data.Remove(rl);
            ...
            var allAsync = async () =>
            {
                await this.readingListService.All(userId, invalidStatus, 1, 2);
            };
        ...
        [Fact]
        public async Task All_ShouldReturnCorrectPagination_WhenPageIndexIsBeyondRange()
        {
            foreach (var rl in this.data.ReadingLists)
            {
                this.data.Remove(rl);
            ...
            var result = await this.readingListService.All(userId, status, 2, 2);
        ...
        [Fact]
        public async Task All_ShouldReturnCorrectPagination_WhenPageSizeIsSmallerThanTotalItems()
        {
            foreach (var rl in this.data.ReadingLists)
            {
                this.data.Remove(rl);
