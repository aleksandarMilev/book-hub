namespace BookHub.Tests.Services
{
    ...
    public class VoteServiceTests
    {
        ...
        [Fact]
        public async Task Create_ShouldReturnNull_WhenVoteAlreadyExists()
        {
            var reviewId = 1;
            var isUpvote = true;
            var userId = "user2Id";
            ...
            var existingVote = await this.data.Votes.FirstOrDefaultAsync(v =>
                v.ReviewId == reviewId &&
                v.CreatorId == userId &&
                v.IsUpvote == isUpvote);
            ...
            if (existingVote == null)
            {
                ...
                this.data.Votes.Add(existingVote);
                await this.data.SaveChangesAsync();
            }

            var result = await this.voteService.Create(reviewId, isUpvote);

            result.Should().BeNull();
        }

        [Fact]
        public async Task Create_ShouldRemoveOppositeVote_WhenOppositeVoteExists()
        {
            var reviewId = 1;
            var isUpvote = false;
            var userId = "user2Id";
        ...
        [Fact]
        public async Task Create_ShouldCreateNewVote_WhenNoVoteExists()
        {
            var reviewId = 1;
            var isUpvote = false;
            var userId = "user1Id";
        ...
        [Fact]
        public async Task Create_ShouldNotCreateVote_WhenReviewIdDoesNotExist()
        {
            ...
            var result = await this.voteService.Create(invalidReviewId, isUpvote);
            result.Should().NotBeNull(); 
