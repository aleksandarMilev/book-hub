namespace BookHub.Features.Authors.Mapper
{
    using AutoMapper;
        ...
        public AuthorMapper()
        {
            ...
            this.CreateMap<Book, BookServiceModel>()
                .ForMember(
                    dest => dest.Genres,
                    opt => opt.MapFrom(
                        src => src.BooksGenres.Select(bg => bg.Genre)));
            ...
            this.CreateMap<CreateAuthorServiceModel, Author>()
                .ForMember(
                    dest => dest.Gender,
                    opt => opt.MapFrom(
                        src => MapperHelper.ParseGender(src.Gender)))
                .ForMember(
                    dest => dest.BornAt,
                    opt => opt.MapFrom(
                        src => MapperHelper.ParseDateTime(src.BornAt)))
                .ForMember(
                    dest => dest.DiedAt,
                    opt => opt.MapFrom(
                        src => MapperHelper.ParseDateTime(src.DiedAt)));
