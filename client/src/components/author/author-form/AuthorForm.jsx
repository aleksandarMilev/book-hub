...
export default function AuthorForm({ authorData = null, isEditMode = false }) {
  ...
  const initialNationality =
    isEditMode && authorData?.nationality ? authorData.nationality.id : "";
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        const finalValues = {
          ...values,
          nationality: values.nationality || initialNationality,
        };

        if (isEditMode) {
          const isSuccessfullyEdited = await editHandler(
            authorData.id,
            finalValues
          );
              ...
              <MDBCol md="6">
                <MDBCardBody className="text-black d-flex flex-column justify-content-center">
                  <h3 className="mb-5 fw-bold">
                    {isEditMode
                      ? "Edit Author"
                      : "Add a New Author to Support Our Community!"}
                  </h3>
