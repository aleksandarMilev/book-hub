...
export async function searchBooksAsync(
  ...
  const url = `${baseUrl}${routes.searchBooks}?searchTerm=${encodeURIComponent(
    searchTerm
...
export async function searchChatsAsync(
  token,
  searchTerm,
  page = pagination.defaultPageIndex,
  pageSize = pagination.defaultPageSize
  ...
  const url = `${baseUrl}${routes.searchChats}?searchTerm=${encodeURIComponent(
    searchTerm
  )}&page=${page}&pageSize=${pageSize}`;
  const response = await fetch(url, options);
